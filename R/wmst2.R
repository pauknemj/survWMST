...
#' @usage  rmst2(time, status, arm, tau0 = NULL, tau1 = NULL, alternative = "two.sided", alpha = 0.05)
...
#' @details For more details, please see the package vignette: \code{browseVignettes(package = "survWM2")}
#' @return an object of class wmst2.
...
#'@export
#########################################
# wmst2 (2-arm) contrast (main function)
#########################################

wmst2 <- function (time, status, arm, tau0 = NULL, tau1 = NULL,
  ...
  if(tau0 != tau1){
    ...
    #--- Milestone Analysis ---
      ...
      wmst = matrix(data=NA,nrow=2,ncol=4)
      rownames(wmst) = c("WMST (arm=1)",
                         "WMST (arm=0)")
